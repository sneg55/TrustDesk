...
CODE_FILES=$(find "$SRC_DIR" -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" -o -name "*.py" -o -name "*.rs" -o -name "*.go" -o -name "*.sol" \) \
  ! -path "*/node_modules/*" ! -path "*/.git/*" ! -path "*/dist/*" ! -path "*/build/*" ! -path "*/__pycache__/*" ! -path "*/target/*" ! -path "*/cache/*" ! -path "*/.venv/*" 2>/dev/null)
...
EOF
  ...
  if [ "$PCT" -lt 64 ]; then
    echo -e "\nBelow 64% target. Prioritize splitting large files." >&2
  fi
fi
