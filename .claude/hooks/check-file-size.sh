...
- constants.py — named constants and config values
- validation.py — input validation logic
- utils.py — pure helper functions
- tests/ — tests (always separate)
...
EOF
  ...
  cat >&2 <<EOF
FILE GETTING LARGE: $FILE_PATH has $LINE_COUNT lines (target: <$WARN_THRESHOLD)

Consider splitting soon. Look for:
- Type definitions -> extract to types.py
- Constants/config -> extract to constants.py
- Validation logic -> extract to validation.py
- Helper functions -> extract to utils.py
